...
void printUnsignedIntWithSeparators(uint value) {
   // idc about bases != 10
   const uint[10] digits = uint[](
         _0, _1, _2, _3, _4, _5, _6, _7, _8, _9
      );
...
void printBar(float value, int width, vec3 col0, vec3 col1) {
   // honestly i botched this code together idk if its reusable anywhere else. i copied and pasted parts of the rest of the library
   ivec2 effectiveSpaceSize = spaceSize + text.charPadding * 2;
   ivec2 pos = text.fragPos - text.textPos - effectiveSpaceSize * text.charPos * ivec2(1, -1) + ivec2(0, effectiveSpaceSize.y);

   int lineH = effectiveSpaceSize.y;
   int totalWidth = width;
   int totalHeight = lineH + 2;

   int shiftedY = pos.y + totalHeight / 2 + 1;

   ivec2 bgPos = ivec2(pos.x, shiftedY) + text.bgPadding;
   ivec2 bgSize = ivec2(totalWidth, totalHeight) + text.bgPadding * 2;
   if (clamp(bgPos, ivec2(0), bgSize - 1) == bgPos) {
      float bgAlpha = text.result.a;
      if (bgAlpha < 0.01) {
         text.result = mix(text.result, text.bgCol, text.bgCol.a);
      }
   ...
   int outerWidth = totalWidth - marginX;
   int outerHeight = totalHeight - marginY;

   int bx = pos.x - marginX;
   int by = shiftedY - marginY;

   if (bx >= 0 && bx < outerWidth && by >= 0 && by < outerHeight) {
      bool isBorder = bx < border || bx >= outerWidth - border || by < border || by >= outerHeight - border;

      if (isBorder) {
         text.result = mix(text.result, text.fgCol, text.fgCol.a);
      } else {
         int innerX = bx - border;
         int innerWidth = outerWidth - (border);
         int innerY = by - border;
         int innerHeight = outerHeight - (border);

         if (innerX >= 0 && innerX < innerWidth && innerY >= 0 && innerY < innerHeight) {
            float progress = float(innerX) / float(innerWidth);
            if (progress < value) {
               vec3 col = mix(col0, col1, progress);
               vec4 fg = vec4(col, text.fgCol.a);
               text.result = mix(text.result, fg, fg.a);
            }
         }
      }
   }
