...
void computeTangentBasis(uint quadID, int triIndex, vec3 geomNormal, out vec3 tangent, out vec3 bitangent) {
   QuadGeometry qg = quadGeometry[quadID];
   QuadAttributes qa = quadAttributes[quadID];
   vec3 p0, p1, p2, p3;
   ...
   vec2 uv0 = qaUV(qa, 0u);
   vec2 uv1 = (triIndex == 0) ? qaUV(qa, 1u) : qaUV(qa, 2u);
   vec2 uv2 = (triIndex == 0) ? qaUV(qa, 2u) : qaUV(qa, 3u);
...
void decodeLabPBR(vec2 uv, vec3 geomNormal, uint quadID, int triIndex, uint textureID, vec3 baseColor, out vec3 normal, out float roughness, out float metallic, out vec3 F0, out vec3 F82tint, out float emission, out float ao, out float sss) {
   vec4 nTexSample;
   vec4 spec;
