...
#ifndef PHOTON_SKY_CLOUDS_ALTOCUMULUS_INCLUDE_GUARD
#define PHOTON_SKY_CLOUDS_ALTOCUMULUS_INCLUDE_GUARD
...
float clouds_altocumulus_density(vec3 pos) {
    ...
    pos.xz += photonCameraPosition.xz * CLOUDS_SCALE + wind_velocity * world_age;

    vec4 noise = vec4(
        // 2D noise for base shape and coverage
        texture(photonCloudNoise, (0.000005 / CLOUDS_ALTOCUMULUS_SIZE) * pos.xz + 0.01)
            .x, // cloud coverage
        texture(photonCloudNoise, (0.000047 / CLOUDS_ALTOCUMULUS_SIZE) * pos.xz + 0.3)
            .wx, // cloud shape
        texture(photonCloudNoise, (0.0001 / CLOUDS_ALTOCUMULUS_SIZE) * pos.xz + 0.6)
            .w // cloud detail
    );
    ...
    float worley
        = texture(colortex10, (pos + 0.2 * wind) * 0.003).x;
#else
...
CloudsResult draw_altocumulus_clouds(
    ...
    float ray_length
        = (distance_to_terrain >= 0.0) ? min(distance_to_terrain, dists.y) : dists.y;
    ...
    float scattering_coeff = extinction_coeff * mix(1.00, 0.75, photonRainStrength);
