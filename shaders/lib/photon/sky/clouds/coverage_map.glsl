...
#ifndef PHOTON_SKY_CLOUDS_COVERAGE_MAP_INCLUDE_GUARD
#define PHOTON_SKY_CLOUDS_COVERAGE_MAP_INCLUDE_GUARD
...
float clouds_cumulus_local_coverage(vec2 pos) {
    ...
    pos += photonCameraPosition.xz * CLOUDS_SCALE;
    ...
    vec2 noise = vec2(
        texture(photonCloudNoise, p1).x, // cloud coverage
        texture(photonCloudNoise, p2).w // cloud shape
    );
...
#endif // PHOTON_SKY_CLOUDS_COVERAGE_MAP_INCLUDE_GUARD
