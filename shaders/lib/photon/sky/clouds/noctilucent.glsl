...
#ifndef PHOTON_SKY_CLOUDS_NOCTILUCENT_INCLUDE_GUARD
#define PHOTON_SKY_CLOUDS_NOCTILUCENT_INCLUDE_GUARD
...
float clouds_noctilucent_density(vec2 coord, vec3 ray_dir) {
    ...
    float density = dampen(texture(photonCloudNoise, 0.000001 * coord + 0.02 * curl).y);
    ...
    for (int i = 0; i < 4; ++i) {
        float detail
            = texture(photonCloudNoise, coord * detail_frequency + curl * curl_strength)
                  .y;
    ...
    float highlight
        = dampen(texture(photonCloudNoise, 0.000004 * coord + 0.02 * curl).y);
    highlight
        -= (1.0 - texture(photonCloudNoise, 0.000014 * coord + 0.05 * curl).y) * 0.15;
...
vec4 draw_noctilucent_clouds(
    ...
    if (distance_to_terrain >= 0.0 && distance_to_terrain < distance_to_sphere) {
        return vec4(0.0, 0.0, 0.0, 1.0);
    }
...
#endif // PHOTON_SKY_CLOUDS_NOCTILUCENT_INCLUDE_GUARD
