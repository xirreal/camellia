...
#ifndef PHOTON_SKY_CLOUDS_CUMULUS_INCLUDE_GUARD
#define PHOTON_SKY_CLOUDS_CUMULUS_INCLUDE_GUARD

// 1st layer: volumetric cumulus/stratocumulus/stratus clouds

#include "/lib/photon/sky/clouds/common.glsl"
#include "/lib/photon/sky/clouds/coverage_map.glsl"
...
float clouds_cumulus_density(vec3 pos) {
    ...
    pos.xz += photonCameraPosition.xz * CLOUDS_SCALE + wind.xz;

    // 3D worley noise for detail
    float worley_0
        = texture(colortex9, (pos + 0.2 * wind) * 0.0009).x;
    float worley_1
        = texture(colortex10, (pos + 0.4 * wind) * 0.005).x;
#else
