...
#ifndef PHOTON_SKY_CLOUDS_COMMON_INCLUDE_GUARD
#define PHOTON_SKY_CLOUDS_COMMON_INCLUDE_GUARD

#include "/lib/photon/bridge.glsl"
#include "/lib/photon/sky/clouds/constants.glsl"
#include "/lib/photon/utility/fast_math.glsl"
#include "/lib/photon/utility/geometry.glsl"
#include "/lib/photon/utility/phase_functions.glsl"
#include "/lib/photon/utility/random.glsl"
#include "/lib/photon/utility/sampling.glsl"
...
vec3 clouds_aerial_perspective(
    ...
    if (length_squared(ray_origin) < length_squared(ray_end)) {
        ...
        air_transmittance = clamp01(trans_0 / max(trans_1, vec3(1e-8)));
    } else {
        vec3 trans_0 = atmosphere_transmittance(ray_origin, -ray_dir);
        vec3 trans_1 = atmosphere_transmittance(ray_end, -ray_dir);

        air_transmittance = clamp01(trans_1 / max(trans_0, vec3(1e-8)));
    }
    ...
    clear_sky = mix(
        clear_sky,
        sky_color * rcp(tau),
        photonRainStrength * mix(1.0, 0.9, time_sunrise + time_sunset)
    );
    air_transmittance
        = mix(air_transmittance, vec3(air_transmittance.x), 0.8 * photonRainStrength);
