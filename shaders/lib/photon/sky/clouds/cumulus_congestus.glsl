...
#ifndef PHOTON_SKY_CLOUDS_CUMULUS_CONGESTUS_INCLUDE_GUARD
#define PHOTON_SKY_CLOUDS_CUMULUS_CONGESTUS_INCLUDE_GUARD

// Alternative 1st layer: distant cumulus congestus clouds

#include "/lib/photon/sky/clouds/common.glsl"
...
float clouds_cumulus_congestus_extinction_coeff = 0.08 * CLOUDS_CUMULUS_CONGESTUS_DENSITY;
#define clouds_cumulus_congestus_scattering_coeff \
    (clouds_cumulus_congestus_extinction_coeff * (1.0 - 0.2 * photonRainStrength))
...
float clouds_cumulus_congestus_density(vec3 pos) {
    ...
    pos.xz += photonCameraPosition.xz * CLOUDS_SCALE;
    ...
    float worley_0
        = texture(colortex10, (pos + 0.2 * wind) * 0.00005).x;
    float worley_1
        = texture(colortex10, (pos + 0.4 * wind) * 0.00023).x;
#else
...
CloudsResult draw_cumulus_congestus_clouds(
    ...
    if (dists.y < 0.0
        ...
        || (distance_to_terrain >= 0.0 && distance_to_terrain < dists.x)) {
        return clouds_not_hit;
    }

    float ray_end = distance_to_terrain >= 0.0 ? min(dists.y, distance_to_terrain) : dists.y;
    float ray_length = max(0.0, ray_end - dists.x);
    ...
    for (uint i = 0u; i < primary_steps; ++i) {
        ...
        vec2 hash = vec2(0.0);
#else
        vec2 hash = vec2(rand(), rand()); // Camellia accumulates light-ray samples over frames
#endif
