...
#ifndef PHOTON_WEATHER_CLOUDS_INCLUDE_GUARD
#define PHOTON_WEATHER_CLOUDS_INCLUDE_GUARD

#include "/lib/photon/sky/clouds/constants.glsl"
#include "/lib/photon/sky/clouds/parameters.glsl"
#include "/lib/photon/weather/core.glsl"
...
float clouds_cirrus_amount(Weather weather) {
    ...
    return clamp01(
               0.5 * temperature_weight * humidity_weight + 0.5 * photonRainStrength
           )
...
float clouds_noctilucent_amount() {
    float intensity = hash1(fract(float(photonWorldDay) * golden_ratio));
...
CloudsParameters get_clouds_parameters(Weather weather) {
    ...
    params.cumulus_congestus_blend = 0.0;
    params.l1_shadow = 0.0;
    ...
    params.l0_extinction_coeff
        ...
        * (1.0 - 0.33 * photonRainStrength) * (1.0 - 0.6 * params.l0_shadow)
    ...
    params.l0_scattering_coeff
        = params.l0_extinction_coeff * mix(1.00, 0.66, photonRainStrength);
...
#endif // PHOTON_WEATHER_CLOUDS_INCLUDE_GUARD
